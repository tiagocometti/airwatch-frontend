...
export class MeasurementsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit()         { this.deviceSvc.getAll().subscribe(d => this.devices.set(d)); this.loadMeasurements(); }
  ...
  loadMeasurements() {
    ...
    const { deviceId, from, to } = this.filterForm.value;
    const obs = deviceId
      ? this.measurementSvc.getByDevice(deviceId, this.currentPage(), 20)
      : (from && to)
  ...
  private renderChart() {
    ...
    const items = [...this.result()!.items].reverse().slice(-30);

    this.chart?.destroy();
    const ctx    = this.chartCanvas.nativeElement.getContext('2d')!;
    const labels = items.map(m => new Date(m.timestamp).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }));
    ...
    this.chart = new Chart(ctx, {
      ...
      data: {
        labels,
        datasets: [
          { label: 'Álcool (ppm)',  data: items.map(m => m.ppmAlcohol), borderColor: '#00d4ff', backgroundColor: 'transparent', tension: 0.4, fill: false, pointRadius: 2 },
          { label: 'GLP (ppm)',     data: items.map(m => m.ppmLpg),     borderColor: '#ff9800', backgroundColor: 'transparent', tension: 0.4, fill: false, pointRadius: 2 },
          { label: 'CO₂ (ppm)',     data: items.map(m => m.ppmCo2),     borderColor: '#00e676', backgroundColor: 'transparent', tension: 0.4, fill: false, pointRadius: 2 },
          { label: 'NH₃ (ppm)',     data: items.map(m => m.ppmNh3),     borderColor: '#ff4081', backgroundColor: 'transparent', tension: 0.4, fill: false, pointRadius: 2 }
        ]
      },
      options: {
  ...
  maxPpm(m: Measurement) { return Math.max(m.ppmAlcohol, m.ppmLpg, m.ppmCo2, m.ppmNh3); }
